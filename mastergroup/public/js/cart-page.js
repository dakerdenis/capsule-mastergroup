(function () {
    ...
    let USER_CPS = parseInt(elPage?.dataset.userCps || "0", 10) || 0;
    ...
    function renderList(items) {
        ...
        <div class="cart__element-desc">
          <div class="cart__element-desc-container">
            ...
            <div class="cart__element-code">${escapeHtml(
                (it.type ? it.type + " - " : "") + (it.code || "")
            )}</div>
          </div>
    ...
    function renderSelected(items) {
        ...
        for (const it of selected) {
            ...
            box.innerHTML = `
        <div class="selected_bin-element-image"><img src="${
            it.image
        }" alt=""></div>
    ...
    async function toggleSelect(pid, btn, toSelected) {
        await post("/cart/select", {
            product_id: pid,
            selected: toSelected ? 1 : 0,
        });
    ...
    function openSuccessModal(orderNumber) {
        ...
        if (els.sOrderNumber)
            els.sOrderNumber.textContent = orderNumber
                ? "#" + String(orderNumber)
                : "";
    ...
    els.sOk?.addEventListener("click", (e) => {
        e.preventDefault();
        closeSuccessModal();
    });
    ...
    async function placeOrder() {
        ...
        if (res.ok && data.ok) {
            try {
                ...
                if (topbarCps && typeof data.new_cps === "number")
                    topbarCps.textContent = String(data.new_cps);
            } catch (e) {}
            // ✅ Обновляем локальный баланс пользователя и дата-атрибут на странице
            if (typeof data.new_cps === "number") {
                USER_CPS = data.new_cps;
                if (elPage) elPage.dataset.userCps = String(USER_CPS);
            }

            // После load() корзина пустая, выбранная сумма = 0
            // На всякий случай «добиваём» UI totals (если что-то кэшировалось)
            updateTotals(0, 0);
